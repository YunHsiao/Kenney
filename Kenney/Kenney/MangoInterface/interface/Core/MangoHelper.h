...
//--------------------------------------------------------------------------------------
#pragma once
#ifndef MANGO_HELPER_H
#define MANGO_HELPER_H
...
// Example usage:
//
//      MANGO_GAMEPAD gamepad[4];
//      for( DWORD iPort=0; iPort<MANGO_MAX_CONTROLLERS; iPort++ )
//          MANGOGetGamepadState( iPort, gamepad[iPort] );
//
//--------------------------------------------------------------------------------------
#define MANGO_MAX_CONTROLLERS 4  // XInput handles up to 4 controllers 

struct MANGO_GAMEPAD
{
	// From XINPUT_GAMEPAD
	WORD    wButtons;
	BYTE    bLeftTrigger;
...
HRESULT MANGOGetGamepadState( DWORD dwPort, MANGO_GAMEPAD* pGamePad, bool bThumbstickDeadZone = true, bool bSnapThumbstickToCardinals = true );
...
{
public:
	CMangoArray()  { m_pData = NULL; m_nSize = 0; m_nMaxSize = 0; }
	CMangoArray( const CMangoArray<TYPE>& a ) { for( int i=0; i < a.m_nSize; i++ ) Add( a.m_pData[i] ); }
	~CMangoArray() { RemoveAll(); }

	const TYPE& operator[]( int nIndex ) const { return GetAt( nIndex ); }
	TYPE& operator[]( int nIndex ) { return GetAt( nIndex ); }

	CMangoArray& operator=( const CMangoArray<TYPE>& a ) { if( this == &a ) return *this; RemoveAll(); for( int i=0; i < a.m_nSize; i++ ) Add( a.m_pData[i] ); return *this; }
...
// Use MANGOGetGlobalTimer() to get the global instance
//--------------------------------------------------------------------------------------
class CMANGOTimer
{
public:
...
// Use MANGOGetGlobalResourceCache() to access the global cache
//-----------------------------------------------------------------------------

enum MANGOCACHE_SOURCELOCATION { MANGOCACHE_LOCATION_FILE, MANGOCACHE_LOCATION_RESOURCE };

struct MANGOCache_Texture
{
	MANGOCACHE_SOURCELOCATION Location;
...
struct MANGOCache_Font : public D3DXFONT_DESC
{
	ID3DXFont *pFont;
};

struct MANGOCache_Effect
{
	MANGOCACHE_SOURCELOCATION Location;
	WCHAR wszSource[MAX_PATH];
	HMODULE hSrcModule;
...
protected:
	friend CMANGOResourceCache& MANGOGetGlobalResourceCache();
	friend HRESULT MANGOInitialize3DEnvironment();
	friend HRESULT MANGOReset3DEnvironment();
	friend void MANGOCleanup3DEnvironment( bool bReleaseSettings );

	CMANGOResourceCache() { }

	CMangoArray< MANGOCache_Texture > m_TextureCache;
	CMangoArray< MANGOCache_Effect > m_EffectCache;
	CMangoArray< MANGOCache_Font > m_FontCache;
};
...
protected:
	...
	MANGO_GAMEPAD          m_GamePad[MANGO_MAX_CONTROLLERS]; // XInput controller state
	D3DXVECTOR3           m_vGamePadLeftThumb;
	D3DXVECTOR3           m_vGamePadRightThumb;
	double                m_GamePadLastActive[MANGO_MAX_CONTROLLERS];
...
//--------------------------------------------------------------------------------------
class CMANGOTextHelper
{
public:
	CMANGOTextHelper( ID3DXFont* pFont, ID3DXSprite* pSprite, int nLineHeight );
...
protected:
	...
	CMangoArray<LINE_NODE*> m_LinesList;
...
//--------------------------------------------------------------------------------------
class CMANGODirectionWidget
{
public:
	CMANGODirectionWidget();
...
HRESULT WINAPI MANGOTrace( const CHAR* strFile, DWORD dwLine, HRESULT hr, const WCHAR* strMsg, bool bPopMsgBox );
...
#ifdef UNICODE
#define MANGOOutputDebugString MANGOOutputDebugStringW
#else
#define MANGOOutputDebugString MANGOOutputDebugStringA
#endif

// These macros are very similar to dxerr's but it special cases the HRESULT defined
// by MANGO to pop better message boxes. 
#if defined(DEBUG) || defined(_DEBUG)
#define MANGO_ERR(str,hr)           MANGOTrace( __FILE__, (DWORD)__LINE__, hr, str, false )
#define MANGO_ERR_MSGBOX(str,hr)    MANGOTrace( __FILE__, (DWORD)__LINE__, hr, str, true )
#define MANGOTRACE                  MANGOOutputDebugString
#else
#define MANGO_ERR(str,hr)           (hr)
#define MANGO_ERR_MSGBOX(str,hr)    (hr)
#define MANGOTRACE                  (__noop)
#endif
...
//--------------------------------------------------------------------------------------
const D3DCOLOR MANGO_PERFEVENTCOLOR  = D3DCOLOR_XRGB(200,100,100);
const D3DCOLOR MANGO_PERFEVENTCOLOR2 = D3DCOLOR_XRGB(100,200,100);
const D3DCOLOR MANGO_PERFEVENTCOLOR3 = D3DCOLOR_XRGB(100,100,200);
...
#define MANGO_BeginPerfEvent( color, pstrMessage )   MANGO_Dynamic_D3DPERF_BeginEvent( color, pstrMessage )
#define MANGO_EndPerfEvent()                         MANGO_Dynamic_D3DPERF_EndEvent()
#define MANGO_SetPerfMarker( color, pstrMessage )    MANGO_Dynamic_D3DPERF_SetMarker( color, pstrMessage )
#else
// PROFILE is not defined, so these macros do nothing
#define MANGO_BeginPerfEvent( color, pstrMessage )   (__noop)
#define MANGO_EndPerfEvent()                         (__noop)
#define MANGO_SetPerfMarker( color, pstrMessage )    (__noop)
#endif

//--------------------------------------------------------------------------------------
// CMANGOPerfEventGenerator is a helper class that makes it easy to attach begin and end
// events to a block of code.  Simply define a CMANGOPerfEventGenerator variable anywhere 
// in a block of code, and the class's constructor will call MANGO_BeginPerfEvent when 
// the block of code begins, and the class's destructor will call MANGO_EndPerfEvent when 
// the block ends.
//--------------------------------------------------------------------------------------
class CMANGOPerfEventGenerator
{
public:
	CMANGOPerfEventGenerator( D3DCOLOR color, LPCWSTR pstrMessage ) { MANGO_BeginPerfEvent( color, pstrMessage ); }
	~CMANGOPerfEventGenerator( void ) { MANGO_EndPerfEvent(); }
};


//--------------------------------------------------------------------------------------
// Multimon handling to support OSes with or without multimon API support.  
// Purposely avoiding the use of multimon.h so MANGO.lib doesn't require 
...
template< typename TYPE >
HRESULT CMangoArray<TYPE>::SetSizeInternal( int nNewMaxSize )
{
	if( nNewMaxSize < 0 )
	{
		assert( false );
...
//--------------------------------------------------------------------------------------
template< typename TYPE >
HRESULT CMangoArray<TYPE>::SetSize( int nNewMaxSize )
{
...
//--------------------------------------------------------------------------------------
template< typename TYPE >
HRESULT CMangoArray<TYPE>::Add( const TYPE& value )
{
	HRESULT hr;
...
//--------------------------------------------------------------------------------------
template< typename TYPE >
HRESULT CMangoArray<TYPE>::Insert( int nIndex, const TYPE& value )
{
	HRESULT hr;
...
//--------------------------------------------------------------------------------------
template< typename TYPE >
HRESULT CMangoArray<TYPE>::SetAt( int nIndex, const TYPE& value )
{
	// Validate arguments
	if( nIndex < 0 ||
		nIndex >= m_nSize )
...
//--------------------------------------------------------------------------------------
template< typename TYPE >
int CMangoArray<TYPE>::IndexOf( const TYPE& value, int iStart, int nNumElements )
{
	// Validate arguments
	if( iStart < 0 || 
		iStart >= m_nSize ||
...
//--------------------------------------------------------------------------------------
template< typename TYPE >
int CMangoArray<TYPE>::LastIndexOf( const TYPE& value, int iEnd, int nNumElements )
{
	// Validate arguments
	if( iEnd < 0 || 
		iEnd >= m_nSize ||
...
//--------------------------------------------------------------------------------------
template< typename TYPE >
HRESULT CMangoArray<TYPE>::Remove( int nIndex )
{
	if( nIndex < 0 || 
		nIndex >= m_nSize )
	{
...
//--------------------------------------------------------------------------------------
IDirect3DDevice9* MANGOCreateRefDevice( HWND hWnd, bool bNullRef = true );


#endif  //MANGO_HELPER_H
