...
//--------------------------------------------------------------------------------------
#include "../Mango.h"
#define MANGO_GAMEPAD_TRIGGER_THRESHOLD      30
...
//--------------------------------------------------------------------------------------
CMANGOResourceCache& MANGOGetGlobalResourceCache()
{
	// Using an accessor function gives control of the construction order
	static CMANGOResourceCache cache;
	return cache;
}
CMANGOTimer* MANGOGetGlobalTimer()
{
	// Using an accessor function gives control of the construction order
	static CMANGOTimer timer;
	return &timer;
}
...
//--------------------------------------------------------------------------------------
bool MANGOFindMediaSearchTypicalDirs( WCHAR* strSearchPath, int cchSearch, LPCWSTR strLeaf, WCHAR* strExePath, WCHAR* strExeName );
bool MANGOFindMediaSearchParentDirs( WCHAR* strSearchPath, int cchSearch, WCHAR* strStartAt, WCHAR* strLeafName );
...
//--------------------------------------------------------------------------------------
void MANGODisplaySwitchingToREFWarning()
{
	if( MANGOGetShowMsgBoxOnError() )
	{
		...
		if( dwSkipWarning == 0 )
		{
			...
			typedef struct { DLGITEMTEMPLATE a; WORD b; WORD c; WORD d; WORD e; WORD f; } MANGO_DLG_ITEM; 
			typedef struct { DLGTEMPLATE a; WORD b; WORD c; WCHAR d[2]; WORD e; WCHAR f[14]; MANGO_DLG_ITEM i1; MANGO_DLG_ITEM i2; MANGO_DLG_ITEM i3; MANGO_DLG_ITEM i4; MANGO_DLG_ITEM i5; } MANGO_DLG_DATA; 
			...
			int nResult = (int) DialogBoxIndirect( MANGOGetHINSTANCE(), (DLGTEMPLATE*)&dtp, MANGOGetHWND(), DisplaySwitchToREFWarningProc ); 
...
//--------------------------------------------------------------------------------------
// MsgProc for MANGODisplaySwitchingToREFWarning() dialog box
//--------------------------------------------------------------------------------------
INT_PTR CALLBACK DisplaySwitchToREFWarningProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam) 
{ 
	switch (message) 
	...
	case WM_INITDIALOG:
		...
		SetWindowText( hDlg, MANGOGetWindowTitle() );
...
//--------------------------------------------------------------------------------------
double CMANGOTimer::GetAbsoluteTime()
{
	LARGE_INTEGER qwTime;
	QueryPerformanceCounter( &qwTime );
...
//--------------------------------------------------------------------------------------
void CMANGOTimer::GetTimeValues( double* pfTime, double* pfAbsoluteTime, float* pfElapsedTime )
{
...
//--------------------------------------------------------------------------------------
double CMANGOTimer::GetElapsedTime()
{
	LARGE_INTEGER qwTime = GetAdjustedCurrentTime();
	...
	// See the explanation about clamping in CMANGOTimer::GetTimeValues()
	if( fElapsedTime < 0.0f )
		fElapsedTime = 0.0f;
...
//--------------------------------------------------------------------------------------
LARGE_INTEGER CMANGOTimer::GetAdjustedCurrentTime()
{
	LARGE_INTEGER qwTime;
	if( m_llStopTime != 0 )
...
//--------------------------------------------------------------------------------------
void CMANGOTimer::LimitThreadAffinityToCurrentProc()
{
...
//--------------------------------------------------------------------------------------
LPCWSTR MANGOGetMediaSearchPath()
{
	return MANGOMediaSearchPath();
}


//--------------------------------------------------------------------------------------
HRESULT MANGOSetMediaSearchPath( LPCWSTR strPath )
{
	HRESULT hr;

	WCHAR* s_strSearchPath = MANGOMediaSearchPath();
...
//--------------------------------------------------------------------------------------
HRESULT MANGOFindDXSDKMediaFileCch( WCHAR* strDestPath, int cchDest, LPCWSTR strFilename )
{
	bool bFound;
	...
	bFound = MANGOFindMediaSearchTypicalDirs( strDestPath, cchDest, strFilename, strExePath, strExeName );
	if( bFound )
		return S_OK;
	...
	bFound = MANGOFindMediaSearchTypicalDirs( strDestPath, cchDest, strSearchFor, strExePath, strExeName );
	if( bFound )
		return S_OK;
	...
	bFound = MANGOFindMediaSearchParentDirs( strDestPath, cchDest, L".", strLeafName );
	if( bFound )
		return S_OK;

	// Search all parent directories starting at the exe's dir and using strFilename as the leaf name
	bFound = MANGOFindMediaSearchParentDirs( strDestPath, cchDest, strExePath, strLeafName );
	if( bFound )
		return S_OK;
...
//--------------------------------------------------------------------------------------
bool MANGOFindMediaSearchTypicalDirs( WCHAR* strSearchPath, int cchSearch, LPCWSTR strLeaf, 
...
CMANGOResourceCache::~CMANGOResourceCache()
{
	OnDestroyDevice();
...
HRESULT CMANGOResourceCache::CreateTextureFromFile( LPDIRECT3DDEVICE9 pDevice, LPCTSTR pSrcFile, LPDIRECT3DTEXTURE9 *ppTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateTextureFromFileEx( LPDIRECT3DDEVICE9 pDevice, LPCTSTR pSrcFile, UINT Width, UINT Height, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DTEXTURE9 *ppTexture )
{
	// Search the cache for a matching entry.
	for( int i = 0; i < m_TextureCache.GetSize(); ++i )
	{
		MANGOCache_Texture &Entry = m_TextureCache[i];
		if( Entry.Location == MANGOCACHE_LOCATION_FILE &&
	...
	MANGOCache_Texture NewEntry;
	NewEntry.Location = MANGOCACHE_LOCATION_FILE;
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateTextureFromResource( LPDIRECT3DDEVICE9 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, LPDIRECT3DTEXTURE9 *ppTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateTextureFromResourceEx( LPDIRECT3DDEVICE9 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, UINT Width, UINT Height, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DTEXTURE9 *ppTexture )
{
	// Search the cache for a matching entry.
	for( int i = 0; i < m_TextureCache.GetSize(); ++i )
	{
		MANGOCache_Texture &Entry = m_TextureCache[i];
		if( Entry.Location == MANGOCACHE_LOCATION_RESOURCE &&
	...
	MANGOCache_Texture NewEntry;
	NewEntry.Location = MANGOCACHE_LOCATION_RESOURCE;
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateCubeTextureFromFile( LPDIRECT3DDEVICE9 pDevice, LPCTSTR pSrcFile, LPDIRECT3DCUBETEXTURE9 *ppCubeTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateCubeTextureFromFileEx( LPDIRECT3DDEVICE9 pDevice, LPCTSTR pSrcFile, UINT Size, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DCUBETEXTURE9 *ppCubeTexture )
{
	// Search the cache for a matching entry.
	for( int i = 0; i < m_TextureCache.GetSize(); ++i )
	{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateCubeTextureFromResource( LPDIRECT3DDEVICE9 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, LPDIRECT3DCUBETEXTURE9 *ppCubeTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateCubeTextureFromResourceEx( LPDIRECT3DDEVICE9 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, UINT Size, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DCUBETEXTURE9 *ppCubeTexture )
{
	// Search the cache for a matching entry.
	for( int i = 0; i < m_TextureCache.GetSize(); ++i )
	{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateVolumeTextureFromFile( LPDIRECT3DDEVICE9 pDevice, LPCTSTR pSrcFile, LPDIRECT3DVOLUMETEXTURE9 *ppVolumeTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateVolumeTextureFromFileEx( LPDIRECT3DDEVICE9 pDevice, LPCTSTR pSrcFile, UINT Width, UINT Height, UINT Depth, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DVOLUMETEXTURE9 *ppTexture )
{
	// Search the cache for a matching entry.
	for( int i = 0; i < m_TextureCache.GetSize(); ++i )
	{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateVolumeTextureFromResource( LPDIRECT3DDEVICE9 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, LPDIRECT3DVOLUMETEXTURE9 *ppVolumeTexture )
{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateVolumeTextureFromResourceEx( LPDIRECT3DDEVICE9 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, UINT Width, UINT Height, UINT Depth, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DVOLUMETEXTURE9 *ppVolumeTexture )
{
	// Search the cache for a matching entry.
	for( int i = 0; i < m_TextureCache.GetSize(); ++i )
	{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateFont( LPDIRECT3DDEVICE9 pDevice, UINT Height, UINT Width, UINT Weight, UINT MipLevels, BOOL Italic, DWORD CharSet, DWORD OutputPrecision, DWORD Quality, DWORD PitchAndFamily, LPCTSTR pFacename, LPD3DXFONT *ppFont )
{
	D3DXFONT_DESCW Desc;
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateFontIndirect( LPDIRECT3DDEVICE9 pDevice, CONST D3DXFONT_DESC *pDesc, LPD3DXFONT *ppFont )
{
	// Search the cache for a matching entry.
	for( int i = 0; i < m_FontCache.GetSize(); ++i )
	{
		MANGOCache_Font &Entry = m_FontCache[i];
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateEffectFromFile( LPDIRECT3DDEVICE9 pDevice, LPCTSTR pSrcFile, const D3DXMACRO *pDefines, LPD3DXINCLUDE pInclude, DWORD Flags, LPD3DXEFFECTPOOL pPool, LPD3DXEFFECT *ppEffect, LPD3DXBUFFER *ppCompilationErrors )
{
	// Search the cache for a matching entry.
	for( int i = 0; i < m_EffectCache.GetSize(); ++i )
	{
		MANGOCache_Effect &Entry = m_EffectCache[i];
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::CreateEffectFromResource( LPDIRECT3DDEVICE9 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, const D3DXMACRO *pDefines, LPD3DXINCLUDE pInclude, DWORD Flags, LPD3DXEFFECTPOOL pPool, LPD3DXEFFECT *ppEffect, LPD3DXBUFFER *ppCompilationErrors )
{
	// Search the cache for a matching entry.
	for( int i = 0; i < m_EffectCache.GetSize(); ++i )
	{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::OnCreateDevice( IDirect3DDevice9 *pd3dDevice )
{
	return S_OK;
}
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::OnResetDevice( IDirect3DDevice9 *pd3dDevice )
{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::OnLostDevice()
{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOResourceCache::OnDestroyDevice()
{
...
CBaseCamera::CBaseCamera()
{
	...
	ZeroMemory( m_GamePad, sizeof(MANGO_GAMEPAD)*MANGO_MAX_CONTROLLERS );
...
void CBaseCamera::GetInput( bool bGetKeyboardInput, bool bGetMouseInput, bool bGetGamepadInput, bool bResetCursorAfterMove )
{
	...
	if( bGetMouseInput )
	{
		...
		if( bResetCursorAfterMove && MANGOIsActive() )
		{
			...
			MANGOGetMonitorInfo( MANGOMonitorFromWindow(MANGOGetHWND(),MONITOR_DEFAULTTONEAREST), &mi );
	...
	if( bGetGamepadInput )
	{
		...
		for( DWORD iUserIndex=0; iUserIndex<MANGO_MAX_CONTROLLERS; iUserIndex++ )
		{
			MANGOGetGamepadState( iUserIndex, &m_GamePad[iUserIndex], true, true );
			...
			{
				m_GamePadLastActive[iUserIndex] = MANGOGetTime();
			}
		}
...
{
	if( MANGOGetGlobalTimer()->IsStopped() )
		fElapsedTime = 1.0f / MANGOGetFPS();
...
//--------------------------------------------------------------------------------------
D3DXMATRIX MANGOGetCubeMapViewMatrix( DWORD dwFace )
{
	D3DXVECTOR3 vEyePt   = D3DXVECTOR3( 0.0f, 0.0f, 0.0f );
...
//--------------------------------------------------------------------------------------
LPCWSTR MANGOD3DFormatToString( D3DFORMAT format, bool bWithPrefix )
{
	WCHAR* pstr = NULL;
	switch( format )
	{
...
//--------------------------------------------------------------------------------------
VOID MANGOOutputDebugStringW( LPCWSTR strMsg, ... )
{
...
//--------------------------------------------------------------------------------------
VOID MANGOOutputDebugStringA( LPCSTR strMsg, ... )
{
#if defined(DEBUG) || defined(_DEBUG)
	CHAR strBuffer[512];
...
//--------------------------------------------------------------------------------------
CMANGOLineManager::CMANGOLineManager()
{
	m_pd3dDevice = NULL;
	m_pD3DXLine = NULL;
}
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOLineManager::OnCreatedDevice( IDirect3DDevice9* pd3dDevice )
{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOLineManager::OnResetDevice()
{
	if( m_pD3DXLine )
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOLineManager::OnRender()
{
	HRESULT hr;
	if( NULL == m_pD3DXLine )
		return E_INVALIDARG;
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOLineManager::OnLostDevice()
{
	if( m_pD3DXLine )
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOLineManager::OnDeletedDevice()
{
	RemoveAllLines();
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOLineManager::AddLine( int* pnLineID, D3DXVECTOR2* pVertexList, DWORD dwVertexListCount, D3DCOLOR Color, float fWidth, float fScaleRatio, bool bAntiAlias )
{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOLineManager::AddRect( int* pnLineID, RECT rc, D3DCOLOR Color, float fWidth, float fScaleRatio, bool bAntiAlias )
{
	if( fWidth > 2.0f )
	{
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOLineManager::RemoveLine( int nLineID )
{
	for( int i=0; i<m_LinesList.GetSize(); i++ )
	{
		LINE_NODE* pLineNode = m_LinesList.GetAt(i);
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOLineManager::RemoveAllLines()
{
	for( int i=0; i<m_LinesList.GetSize(); i++ )
	{
		LINE_NODE* pLineNode = m_LinesList.GetAt(i);
...
//--------------------------------------------------------------------------------------
CMANGOTextHelper::CMANGOTextHelper( ID3DXFont* pFont, ID3DXSprite* pSprite, int nLineHeight )
{
	m_pFont = pFont;
	m_pSprite = pSprite;
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOTextHelper::DrawFormattedTextLine( const WCHAR* strMsg, ... )
{
	WCHAR strBuffer[512];
...
//--------------------------------------------------------------------------------------
HRESULT CMANGOTextHelper::DrawTextLine( const WCHAR* strMsg )
{
	if( NULL == m_pFont ) 
		return MANGO_ERR_MSGBOX( L"DrawTextLine", E_INVALIDARG );
...
HRESULT CMANGOTextHelper::DrawFormattedTextLine( RECT &rc, DWORD dwFlags, const WCHAR* strMsg, ... )
{
	WCHAR strBuffer[512];
...
HRESULT CMANGOTextHelper::DrawTextLine( RECT &rc, DWORD dwFlags, const WCHAR* strMsg )
{
	if( NULL == m_pFont ) 
		return MANGO_ERR_MSGBOX( L"DrawTextLine", E_INVALIDARG );
...
//--------------------------------------------------------------------------------------
void CMANGOTextHelper::Begin()
{
	if( m_pSprite )
		m_pSprite->Begin( D3DXSPRITE_ALPHABLEND | D3DXSPRITE_SORT_TEXTURE );
}
void CMANGOTextHelper::End()
{
	if( m_pSprite )
		m_pSprite->End();
}
...
//--------------------------------------------------------------------------------------
IDirect3DDevice9* CMANGODirectionWidget::s_pd3dDevice = NULL;
ID3DXEffect*      CMANGODirectionWidget::s_pEffect = NULL;       
ID3DXMesh*        CMANGODirectionWidget::s_pMesh = NULL;    


//--------------------------------------------------------------------------------------
CMANGODirectionWidget::CMANGODirectionWidget()
{
	m_fRadius = 1.0f;
...
//--------------------------------------------------------------------------------------
HRESULT CMANGODirectionWidget::StaticOnCreateDevice( IDirect3DDevice9* pd3dDevice )
{
	HRESULT hr;
	...
	//V_RETURN( MANGOCreateArrowMeshFromInternalArray( s_pd3dDevice, &s_pMesh ) );
...
//--------------------------------------------------------------------------------------
HRESULT CMANGODirectionWidget::OnResetDevice( const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
...
//--------------------------------------------------------------------------------------
void CMANGODirectionWidget::StaticOnLostDevice()
{
	if( s_pEffect )
...
//--------------------------------------------------------------------------------------
void CMANGODirectionWidget::StaticOnDestroyDevice()
{
...
//--------------------------------------------------------------------------------------
LRESULT CMANGODirectionWidget::HandleMessages( HWND hWnd, UINT uMsg, 
...
//--------------------------------------------------------------------------------------
HRESULT CMANGODirectionWidget::OnRender( D3DXCOLOR color, const D3DXMATRIX* pmView, 
...
//--------------------------------------------------------------------------------------
HRESULT CMANGODirectionWidget::UpdateLightDir()
{
	D3DXMATRIX mInvView;
	D3DXMatrixInverse(&mInvView, NULL, &m_mView);
	mInvView._41 = mInvView._42 = mInvView._43 = 0;
...
static bool MANGO_EnsureD3DAPIs( void )
{
...
IDirect3D9 * WINAPI MANGO_Dynamic_Direct3DCreate9(UINT SDKVersion) 
{
	if( MANGO_EnsureD3DAPIs() && s_DynamicDirect3DCreate9 != NULL )
		return s_DynamicDirect3DCreate9( SDKVersion );
	else
		return NULL;
}

int WINAPI MANGO_Dynamic_D3DPERF_BeginEvent( D3DCOLOR col, LPCWSTR wszName )
{
	if( MANGO_EnsureD3DAPIs() && s_DynamicD3DPERF_BeginEvent != NULL )
		return s_DynamicD3DPERF_BeginEvent( col, wszName );
	else
		return -1;
}

int WINAPI MANGO_Dynamic_D3DPERF_EndEvent( void )
{
	if( MANGO_EnsureD3DAPIs() && s_DynamicD3DPERF_EndEvent != NULL )
		return s_DynamicD3DPERF_EndEvent();
	else
		return -1;
}

void WINAPI MANGO_Dynamic_D3DPERF_SetMarker( D3DCOLOR col, LPCWSTR wszName )
{
	if( MANGO_EnsureD3DAPIs() && s_DynamicD3DPERF_SetMarker != NULL )
		s_DynamicD3DPERF_SetMarker( col, wszName );
}

void WINAPI MANGO_Dynamic_D3DPERF_SetRegion( D3DCOLOR col, LPCWSTR wszName )
{
	if( MANGO_EnsureD3DAPIs() && s_DynamicD3DPERF_SetRegion != NULL )
		s_DynamicD3DPERF_SetRegion( col, wszName );
}

BOOL WINAPI MANGO_Dynamic_D3DPERF_QueryRepeatFrame( void )
{
	if( MANGO_EnsureD3DAPIs() && s_DynamicD3DPERF_QueryRepeatFrame != NULL )
		return s_DynamicD3DPERF_QueryRepeatFrame();
	else
		return FALSE;
}

void WINAPI MANGO_Dynamic_D3DPERF_SetOptions( DWORD dwOptions )
{
	if( MANGO_EnsureD3DAPIs() && s_DynamicD3DPERF_SetOptions != NULL )
		s_DynamicD3DPERF_SetOptions( dwOptions );
}

DWORD WINAPI MANGO_Dynamic_D3DPERF_GetStatus( void )
{
	if( MANGO_EnsureD3DAPIs() && s_DynamicD3DPERF_GetStatus != NULL )
...
//--------------------------------------------------------------------------------------
void MANGOTraceDecl( D3DVERTEXELEMENT9 decl[MAX_FVF_DECL_SIZE] )
{
	int iDecl=0;
	for( iDecl=0; iDecl<MAX_FVF_DECL_SIZE; iDecl++ )
	{
		...
		MANGOOutputDebugString( L"decl[%d]=Stream:%d, Offset:%d, %s, %s, %s, UsageIndex:%d\n", iDecl, 
			decl[iDecl].Stream,
			decl[iDecl].Offset,
			MANGOTraceD3DDECLTYPEtoString( decl[iDecl].Type ),
			MANGOTraceD3DDECLMETHODtoString( decl[iDecl].Method ),
			MANGOTraceD3DDECLUSAGEtoString( decl[iDecl].Usage ),
			decl[iDecl].UsageIndex );
	}

	MANGOOutputDebugString( L"decl[%d]=D3DDECL_END\n", iDecl );
}


//--------------------------------------------------------------------------------------
WCHAR* MANGOTraceD3DDECLTYPEtoString( BYTE t )
{
	switch( t )
	{
...
WCHAR* MANGOTraceD3DDECLMETHODtoString( BYTE m )
{
	switch( m )
	{
...
WCHAR* MANGOTraceD3DDECLUSAGEtoString( BYTE u )
{
	switch( u )
	{
...
//--------------------------------------------------------------------------------------
#define MANGO_PRIMARY_MONITOR ((HMONITOR)0x12340042)
typedef HMONITOR (WINAPI* LPMONITORFROMWINDOW)(HWND, DWORD);
typedef BOOL     (WINAPI* LPGETMONITORINFO)(HMONITOR, LPMONITORINFO);

BOOL MANGOGetMonitorInfo(HMONITOR hMonitor, LPMONITORINFO lpMonitorInfo)
{
	...
	RECT rcWork;
	if ((hMonitor == MANGO_PRIMARY_MONITOR) && lpMonitorInfo && (lpMonitorInfo->cbSize >= sizeof(MONITORINFO)) && SystemParametersInfoA(SPI_GETWORKAREA, 0, &rcWork, 0))
	{
...
HMONITOR MANGOMonitorFromWindow(HWND hWnd, DWORD dwFlags)
{
	static bool s_bInited = false;
...
//--------------------------------------------------------------------------------------
void MANGOGetDesktopResolution( UINT AdapterOrdinal, UINT* pWidth, UINT* pHeight )
{
	CD3DEnumeration* pd3dEnum = MANGOGetEnumeration();
...
//--------------------------------------------------------------------------------------
IDirect3DDevice9* MANGOCreateRefDevice( HWND hWnd, bool bNullRef )
{
	HRESULT hr;
	IDirect3D9* pD3D = MANGO_Dynamic_Direct3DCreate9( D3D_SDK_VERSION );
	if( NULL == pD3D )
		return NULL;
...
//--------------------------------------------------------------------------------------
HRESULT MANGOGetGamepadState( DWORD dwPort, MANGO_GAMEPAD* pGamePad, bool bThumbstickDeadZone, bool bSnapThumbstickToCardinals )
{
	if( dwPort >= MANGO_MAX_CONTROLLERS || pGamePad == NULL )
		return E_FAIL;
	...
	{
		ZeroMemory( pGamePad, sizeof(MANGO_GAMEPAD) );
	...
	bool bPressed = ( pGamePad->bLeftTrigger > MANGO_GAMEPAD_TRIGGER_THRESHOLD );
	...
	bPressed = ( pGamePad->bRightTrigger > MANGO_GAMEPAD_TRIGGER_THRESHOLD );
...
//--------------------------------------------------------------------------------------
void MANGOEnableXInput( bool bEnable )
{
...
// the controller will continue to rumble
//--------------------------------------------------------------------------------------
HRESULT MANGOStopRumbleOnAllControllers()
{
	...
	for( int iUserIndex=0; iUserIndex<MANGO_MAX_CONTROLLERS; iUserIndex++ )
